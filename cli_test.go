	...
	for _, tc := range cases {
		t.Run(tc.Last, func(t *testing.T) {
			...
			cli := &CLI{
				...
				Autocomplete: true,
				AutocompleteGlobalFlags: map[string]kamili.Predictor{
					"-tubes": kamili.PredictNothing,
				},
			}
...
func TestCLIAutocomplete_subcommandArgs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.Last, func(t *testing.T) {
			...
			command.AutocompleteArgsValue = kamili.PredictFiles("*")
			command.AutocompleteFlagsValue = map[string]kamili.Predictor{
				"-go": kamili.PredictFunc(func(kamili.Args) []string {
					return []string{"yo"}
				}),
			}
			...
			actual := cli.autocomplete.Command.Predict(kamili.Args{
