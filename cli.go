...
import (
	...
	"github.com/wondenge/msingi/radix"
	"io"
	"io/ioutil"
	"os"
	"regexp"
...
type CLI struct {
	...
	AutocompleteGlobalFlags    kamili.Flags
...
func (c *CLI) initAutocomplete() {
	...
	c.autocomplete = kamili.New(c.Name, kamili.Command{})
	...
	if !c.AutocompleteNoDefaultFlags {
		cmd.Flags = map[string]kamili.Predictor{
			"-" + c.AutocompleteInstall:   kamili.PredictNothing,
			"-" + c.AutocompleteUninstall: kamili.PredictNothing,
			"-help":                       kamili.PredictNothing,
			"-version":                    kamili.PredictNothing,
		}
	}
	cmd.GlobalFlags = c.AutocompleteGlobalFlags

	c.autocomplete = kamili.New(c.Name, cmd)
}
...
func (c *CLI) initAutocompleteSub(prefix string) kamili.Command {
	var cmd kamili.Command
	walkFn := func(k string, raw interface{}) bool {
		...
		if cmd.Sub == nil {
			cmd.Sub = kamili.Commands(make(map[string]kamili.Command))
		}
